...
/*Added code to find the determinant of a 3x3 matrix*/
real_t determinant(const Matrix3& m)
{
	real_t v0 = m._m[1][1] * m._m[2][2] - m._m[1][2] * m._m[2][1];
    real_t v1 = m._m[1][2] * m._m[2][0] - m._m[1][0] * m._m[2][2];
    real_t v2 = m._m[1][0] * m._m[2][1] - m._m[1][1] * m._m[2][0];
...
/*Solves the Linear System of Equation Ax=B*/
bool solveLinearSystem(const Matrix3& A, const Vector3& b,Vector3& sol )
{
	real_t dA= determinant(A);
	if(fabs(dA)<1e-11)
		return false;
